...
impl Value {
    ...
    pub fn can_drop(&self) -> PartialVMResult<()> {
        self.0.has_ability(Ability::Drop)
    }

    pub fn has_ability(&self, ability: Ability) -> PartialVMResult<()> {
        self.0.has_ability(ability)
    }
...
impl ValueImpl {
    ...
    fn has_ability(&self, ability: Ability) -> PartialVMResult<()> {
        match self {
            ValueImpl::Address(_)
            | ValueImpl::Bool(_)
            | ValueImpl::U8(_)
            | ValueImpl::U64(_)
            | ValueImpl::U128(_) => check_ability(AbilitySet::PRIMITIVES, ability),
            ValueImpl::Invalid | ValueImpl::ContainerRef(_) | ValueImpl::IndexedRef(_) => {
                check_ability(AbilitySet::REFERENCES, ability)
            }
            ValueImpl::Container(c) => c.has_ability(ability),
        }
    }
...
fn check_ability(ability_set: AbilitySet, ability: Ability) -> PartialVMResult<()> {
    if ability_set.has_ability(ability) {
        Ok(())
    } else {
        Err(
            PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR).with_message(
    ...
    fn has_ability(&self, ability: Ability) -> PartialVMResult<()> {
        match self {
            ...
            Container::Vec(v) => {
                ...
                v.borrow()[0].has_ability(ability)
            }
            Container::VecAddress(_)
            | Container::VecU8(_)
            | Container::VecU64(_)
...
impl IndexedRef {
    ...
    fn has_ability(&self, ability: Ability) -> PartialVMResult<()> {
        ...
        match self.container_ref.container() {
            Locals(r) | Vec(r) => r.borrow()[self.idx].has_ability(ability),
            Struct(r) => r.borrow().0[self.idx].has_ability(ability),
            VecU8(_) | VecU64(_) | VecU128(_) | VecBool(_) | VecAddress(_) => {
                check_ability(AbilitySet::PRIMITIVES, ability)
            }
        }
    }
}
...
impl Locals {
    pub fn check_valid_drop(&self) -> PartialVMResult<()> {
        for value in self.0.borrow().iter() {
            value.has_ability(Ability::Drop)?;
        }
        Ok(())
    }
...
impl Reference {
    pub fn can_overwrite(&self) -> PartialVMResult<()> {
        match &self.0 {
            ReferenceImpl::ContainerRef(container) => {
                container.container().has_ability(Ability::Drop)
            }
            ReferenceImpl::IndexedRef(indexed_ref) => indexed_ref.has_ability(Ability::Drop),
        }
    }
}

impl GlobalValue {
    pub fn check_store(&self) -> PartialVMResult<()> {
        match &self.0 {
            GlobalValueImpl::Deleted | GlobalValueImpl::None => Ok(()),
            GlobalValueImpl::Cached { fields, status: _ } | GlobalValueImpl::Fresh { fields } => {
                if let Some(ability_set) = fields.borrow().2 {
                    check_ability(ability_set, Ability::Key)?;
                }
                for value in fields.borrow().0.iter() {
                    value.has_ability(Ability::Store)?;
                }
                Ok(())
            }
        }
